...
Verified deterministic checks report 5/5 security boundaries, 4/4 multi-operation portal behaviors, 5/5 capability output contracts, and 3/3 router lifecycle tests. Live browser runs verify end-to-end discovery/replay, one-step recovery, bounded hard failure, `MEMBER_NOT_FOUND`, and one same-session handoff. Sub-account and deposit evidence reaches review without commitment. The deliberate core invariant is preserved: the LLM discovers unknown work once, a draft is reviewed, approved capabilities replay without the LLM, external policy governs, and humans control consequential actions.
