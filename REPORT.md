...
Verified deterministic checks report 5/5 security boundaries, 5/5 hardening checks, 4/4 portal behaviors, 5/5 output contracts, and 3/3 router lifecycle tests. The canonical artifact and a privacy-redacted operator-supplied transcript from the documented model-driven command are committed with a provenance manifest. A fresh discovery rerun requires the evaluator's own model API key. Live browser runs verify replay, recovery, bounded hard failure, `MEMBER_NOT_FOUND`, and same-session handoff. The core invariant remains: the LLM discovers unknown work once, a draft is reviewed, approved capabilities replay without the LLM, external policy governs, and humans control consequential actions.
